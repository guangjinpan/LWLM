...
# User rows 1-100
parameters['user_rows'] = np.arange(3) # change 3 to generate more pretrain data
parameters['user_subsampling'] = 1 # Subsample users to 1% of the original number
...
for BW_i in range(len(BW_list)):
    ...
    print("The number of generated coordinates:", len(dataset[0]['user']['channel']))
    ...
    for BS_i in range(len(dataset)):
        ...
        for ue_i in range(len(dataset[BS_i]['user']['channel'])):
            # print(ue_i)
            file_name= "../../LWLM/dataset/O1_3p5/train"


            os.makedirs(file_name+f"/BS{bs_i}_{parameters['OFDM']['subcarriers']}sc_{N_antenna}at_{BW}M/", exist_ok=True)
