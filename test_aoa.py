...
EnvPara["fstride"]  = 4
EnvPara["tstride"]  = 4
EnvPara["task"]  = "inference_aoa" 
EnvPara["load_pretrained_mdl_path"] =  '../../pretrained_model/aoa/mix/nofrozen/10000/testepoch=953.ckpt' #'../../pretrained_model/cnn/aoa/10000/testepoch=983.ckpt' 
EnvPara["save_result_file"] = 'AOA-mix-10M-10000-vvx0'
...
if __name__ == '__main__':   
    ...
    result = np.zeros((500000,1))
    label = np.zeros((500000,1))
    mae = np.zeros((500000,1))
    cnt = 0
    ...
    for  channel_Antenna_subcarrier_aa, channel_Antenna_subcarrier_ri, channel_delay_angle_ri, UElocation_all, BSconf_all in test_dataloader:
        ...
        result[cnt : cnt+len(channel_Antenna_subcarrier_aa)] = pred.cpu().detach().numpy() * 100
        label[cnt : cnt+len(channel_Antenna_subcarrier_aa),0] = UElocation_all[:,0, 3].cpu().detach().numpy() * 100
        mae[cnt : cnt+len(channel_Antenna_subcarrier_aa),0] = loss.cpu().detach().numpy()
    ...
    distances = valid_result - valid_label
